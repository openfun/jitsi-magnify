...
from magnify.rooms.factories import UserFactory
...
class RedirectTestCase(TestCase):
    ...
    def test_redirect_anonymous_user(self):
        """
        Get path /rooms/token/test and redirect to jitsi with a token as a guest
        We thus verify that this redirects as wanted, and that the token is correct
        """
        # Fetch response from "rooms/token/{room}"
        now = datetime(2030, 6, 15, tzinfo=timezone.utc)
        with mock.patch.object(timezone, "now", return_value=now):
            response = self.client.get("/rooms/token/test")
    ...
    def test_redirect_authenticated_user(self):
        """
        Get path /rooms/token/test and redirect to jitsi with a token as a logged in user
        We thus verify that this redirects as wanted, and that the token is correct
        """
        # Log in a user
