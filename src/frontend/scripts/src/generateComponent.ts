...
const notInCamelCase = givenComponentDirs.filter(
  (folder) =>
    folder[0] !== folder[0].toUpperCase() || folder !== folder.replace(/[^a-zA-Z0-9]/g, ''),
);
...
const pathToComponentFolderInComponents = path.join(pathToComponents, ...componentDirsNames);

// CREATE FOLDER
createDirectory(pathToComponentFolderInComponents);

// CREATE INDEX
writeFile(path.join(pathToComponentFolderInComponents, 'index.tsx'), indexTemplate(componentName));
...
appendToFile(
  path.join(indexesFolder[indexesFolder.length - 1], 'index.tsx'),
  defaultAsTemplate(componentName),
);

// Append 'export all' export to others
for (let i = 0; i < indexesFolder.length - 1; i++) {
  const indexPath = path.join(indexesFolder[i], 'index.tsx');
  ...
  try {
    ...
    console.log(`Could not search for ${exportAll} in ${indexPath}: ${error.message}`);
  }
