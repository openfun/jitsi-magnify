...
// Transparently includes Magnify default translation files to lighten command
const LOCALES_PATTERN = path.resolve(__dirname, `./locales/*.json`);
// Output directory
const OUT_DIR = yargs.argv.outDir || path.resolve(__dirname, `../src/translations`);
...
(async () => {
  ...
  Object.entries(files).forEach(([filename, paths]) => {
    ...
    // If a translation contains several objects, we merge them
    if (jsons.length > 0) {
