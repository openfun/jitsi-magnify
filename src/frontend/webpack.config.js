...
module.exports = (env) => {
  const magnifySettings = env ? env['magnify-settings'] : undefined;
  const magnifyDependentBuild = env ? !!env['magnify-dependent-build'] : false;

  // Specific path resolutions for builds ran by a Magnify dependent
  const magnifyDependentModuleResolutions = magnifyDependentBuild
    ? [
        // Magnify dependent node modules
        path.resolve(__dirname, '..'),
        // Magnify dependent's own code
        path.resolve(__dirname, '..', '..'),
      ]
    : [];

  // Load the settings file if its path was passed as an argument and use its overrides
  const overrides = magnifySettings
    ? require(path.join(process.cwd(), magnifySettings)).overrides
    : {};
  ...
  if (magnifyDependentBuild) {
    version = require(path.resolve(__dirname, '..', '..', 'package.json')).version;
  }
  // Otherwise get it from Magnify itself
  else {
    version = require(path.resolve(__dirname, 'package.json')).version;
  }
  ...
  const config = {
    ...
    output: {
      filename: 'index.js',
      path: path.join(__dirname, '/../magnify/static/magnify/js/build'),
    ...
    resolve: {
      ...
      modules: [
        ...
        __dirname,
        'node_modules',
        ...magnifyDependentModuleResolutions,
      ],
    },
    ...
    module: {
      rules: [
        ...
        {
          exclude: /node_modules(?!\/magnify-education)/,
          test: /\.(tsx?|jsx?)$/,
          use: [
            {
              loader: 'babel-loader',
    ...
    plugins: [
      // Use module replacement to override any number of Magnify components as defined in the settings
      ...
      new webpack.DefinePlugin({
        MAGNIFY_VERSION: `'${version.toString()}'`,
      }),
    ],
  };
