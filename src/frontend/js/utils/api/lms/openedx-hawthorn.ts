...
const API = (APIConf: AuthenticationBackend | LMSBackend, options?: APIOptions): APILms => {
  ...
  return {
    user: {
      ...
      /* 
        / ! \ Prefix next param with magnify.
        In this way, OpenEdX Nginx conf knows that we want to go back to magnify app after login/redirect
      */
      login: () => location.assign(`${ROUTES.user.login}?next=magnify${location.pathname}`),
      register: () => location.assign(`${ROUTES.user.register}?next=magnify${location.pathname}`),
      logout: async () => {
