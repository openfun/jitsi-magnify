...
import { REACT_QUERY_SETTINGS, MAGNIFY_USER_TOKEN } from 'settings';
...
describe('JoanieSessionProvider', () => {
  ...
  it('stores user access token within session storage', async () => {
    ...
    expect(sessionStorage.getItem(MAGNIFY_USER_TOKEN)).toEqual(user.access_token);
  });
  ...
  it('clears session storage on login', async () => {
    ...
    expect(sessionStorage.getItem(MAGNIFY_USER_TOKEN)).toBeNull();
