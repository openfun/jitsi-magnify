...
describe('<Root />', () => {
  ...
  it('finds all magnify-react containers and renders the relevant components into them with their passed props', async () => {
    // Create the containers for the two components we're about to render
    const userLoginContainer = document.createElement('div');
    userLoginContainer.setAttribute('class', 'magnify-react magnify-react--user-login');
    ...
    rootSearchSuggestFieldContainer.setAttribute(
      'class',
      'magnify-react magnify-react--root-search-suggest-field',
    );
    rootSearchSuggestFieldContainer.setAttribute(
      'data-props',
    ...
    render(
      <IntlProvider locale="en">
        <Root magnifyReactSpots={[userLoginContainer, rootSearchSuggestFieldContainer]} />
      </IntlProvider>,
    );
  ...
  it('prints a console warning and still renders everything else when it fails to find a component', async () => {
    ...
    userLoginContainer.setAttribute('class', 'magnify-react magnify-react--user-login');
    document.body.append(userLoginContainer);
    // On the other hand, <UserFeedback /> is not a component that exists in Magnify
    const userFeedbackContainer = document.createElement('div');
    userFeedbackContainer.setAttribute('class', 'magnify-react magnify-react--user-feedback');
    document.body.append(userFeedbackContainer);

    // Render the root component, passing our real element and our bogus one
    render(
      <IntlProvider locale="en">
        <Root magnifyReactSpots={[userFeedbackContainer, userLoginContainer]} />
      </IntlProvider>,
    );
  ...
  it('renders properly components even if one of them raises an error', async () => {
    ...
    searchFailingComponent.setAttribute('class', 'magnify-react magnify-react--search');
    document.body.append(searchFailingComponent);

    // Render the root component, passing our real element and our bogus one
    render(
      <IntlProvider locale="en">
        <Root magnifyReactSpots={[userLoginContainer, searchFailingComponent]} />
      </IntlProvider>,
    );
