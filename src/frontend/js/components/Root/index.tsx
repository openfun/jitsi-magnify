...
interface RootProps {
  magnifyReactSpots: Element[];
}

export const Root = ({ magnifyReactSpots }: RootProps) => {
  const portals = magnifyReactSpots.map((element: Element) => {
    ...
    const componentName = startCase(
      get(element.className.match(/magnify-react--([a-zA-Z-]*)/), '[1]') || '',
    )
      .split(' ')
      .join('');
