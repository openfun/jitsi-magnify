...
// regardless of their use throughout the Magnify codebase itself.
...
async function render() {
  ...
  const magnifyReactSpots = Array.prototype.slice.call(document.querySelectorAll('.magnify-react'));

  // Only move on with anything if there are actually components to render
  if (magnifyReactSpots.length) {
    ...
    try {
      ...
      // Magnify implicitly uses 'en-US' as its default locales: it does not provide localization files and strings
      ...
      // Magnify. We still want errors if another locale is expected but missing, and we still want to log something
      // to the console so Magnify users who provide their own 'en-US' strings are warned if they are not loaded.
      if (locale === 'en-US') {
    ...
    reactRoot.setAttribute('class', 'magnify-react magnify-react--root');
    ...
    ReactDOM.render(
      <QueryClientProvider client={queryClient}>
        <IntlProvider locale={locale} messages={translatedMessages} defaultLocale="en-US">
          <Root magnifyReactSpots={magnifyReactSpots} />
        </IntlProvider>
      </QueryClientProvider>,
      reactRoot,
    );
...
// In some case, you would like to render Magnify components manually
window.__MAGNIFY__ = Object.create({ render });
